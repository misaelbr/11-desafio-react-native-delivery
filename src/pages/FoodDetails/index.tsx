...
const FoodDetails: React.FC = () => {
  ...
  function handleIncrementExtra(id: number): void {
    ...
    const findIndex = extras.findIndex(extra => extra.id === id);
    const updateExtras = extras;

    let extra = updateExtras[findIndex];

    extra = {
      ...extra,
      quantity: extra.quantity + 1,
    };
    updateExtras[findIndex] = extra;
  ...
  const cartTotal = useMemo(() => {
    // Calculate cartTotal
    const extraTotal = extras.reduce((sum, extra) => {
      return sum + extra.quantity * extra.value;
    }, 0);

    const foodTotal = (food.price + extraTotal) * foodQuantity;
